...
func (Note *Notes) NewNumberedList(Style string) *List {
	...
	ParaStyle_name := "PS" + strconv.Itoa(rand.Intn(100))
	...
	if len(strings.TrimSpace(Style)) == 0 {
		...
		styletext := `<styles>
	        ...
	        <style:style style:name="` + ParaStyle_name + `" style:family="paragraph" style:parent-style-name="Text_20_body" style:list-style-name="` + ListStyle_name + `">
	...
	StyleNodeid, _ = xmlDB.GetNode(Note.Content, Note.Officeautostyleid, "style:style[style:name=\""+ParaStyle_name+"\"]")
	List.ParaStyleId = StyleNodeid[0]
...
func (List *List) ParaStyle() string {
	Style_name := xmlDB.GetNodeAttribute(List.Note.Content, List.ParaStyleId, "style:name")
	return Style_name
}
...
func (List *List) AddItem(text string) (*Paragraph, *TextSpan) {
	para := List.Note.NewParagraph(List.ParaStyle())
	textspan := para.AddText(text, "")
